...
# ── Pre-install required packages into ISO ────────────────────────────────
log_info "Pre-installing packages into ISO..."

# Create local package cache inside ISO
APK_CACHE="$ISO_ROOT/var/cache/apk"
APK_PACKAGES_DIR="$ISO_ROOT/platine/packages"
mkdir -p "$APK_CACHE" "$APK_PACKAGES_DIR"

# Download packages directly into the ISO
PACKAGES="bash curl dmidecode smartmontools pciutils usbutils lm-sensors hdparm wpa_supplicant grep"

for pkg in $PACKAGES; do
    log_info "Fetching package: $pkg"
    if cmd wget; then
        wget -q --timeout=15 \
            "https://dl-cdn.alpinelinux.org/alpine/v3.19/main/x86_64/${pkg}-"*.apk \
            -P "$APK_PACKAGES_DIR/" 2>/dev/null || \
        wget -q --timeout=15 \
            "https://dl-cdn.alpinelinux.org/alpine/v3.19/community/x86_64/${pkg}-"*.apk \
            -P "$APK_PACKAGES_DIR/" 2>/dev/null || true
    fi
done

# Write install script that runs from local cache
cat > "$ISO_ROOT/platine/install-tools.sh" << 'INSTALLEOF'
#!/bin/sh
# Install pre-bundled tools from local cache
...
apk add --no-cache bash curl dmidecode smartmontools pciutils usbutils lm-sensors hdparm wpa_supplicant grep 2>/dev/null || \
# Fallback: install from local packages
apk add --no-cache --allow-untrusted /media/usb/platine/packages/*.apk 2>/dev/null || true
INSTALLEOF
chmod +x "$ISO_ROOT/platine/install-tools.sh"

log_ok "Package installer ready"
