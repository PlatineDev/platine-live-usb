...
install_tools() {
    local need=()
    cmd lshw      || need+=("lshw")
    cmd smartctl  || need+=("smartmontools")
    cmd dmidecode || need+=("dmidecode")
    cmd sensors   || need+=("lm-sensors")
    cmd lspci     || need+=("pciutils")
    cmd lsusb     || need+=("usbutils")
    cmd upower    || need+=("upower")
    cmd hdparm    || need+=("hdparm")

    if [ ${#need[@]} -gt 0 ]; then
        log_info "Installing: ${need[*]}"
        if   cmd apk;     then apk add --no-cache "${need[@]}" 2>/dev/null || true
        elif cmd apt-get; then apt-get install -y -q "${need[@]}" 2>/dev/null || true
        elif cmd pacman;  then pacman -S --noconfirm "${need[@]}" 2>/dev/null || true
        elif cmd dnf;     then dnf install -y "${need[@]}" 2>/dev/null || true
        fi
    fi
}
...
# Extract live link from response
if [ -n "$RESPONSE" ]; then
    LIVE_LINK=$(echo "$RESPONSE" | grep -oP '"live_url"\s*:\s*"\K[^"]+' || echo "")
fi
...
if [ -n "$LIVE_LINK" ]; then
    ...
    if [ -n "$SESSION_ID" ]; then
        ...
        while true; do
            ...
            if cmd curl; then
                curl -s -m 5 -X POST "$PLATINE_UPDATE" \
                    -H "Content-Type: application/json" \
