...
FIRMWARE_BASE="https://git.kernel.org/pub/scm/linux/kernel/git/firmware/linux-firmware.git/plain/rtlwifi"
FIRMWARES="rtl8723defw.bin rtl8723de_NVM.map rtl8723fw.bin rtl8723fw_B.bin rtl8192defw.bin rtl8192cufw.bin rtl8192cufw_TMSC.bin"

for fw in $FIRMWARES; do
    if cmd wget; then
        wget -q "$FIRMWARE_BASE/$fw" -O "$FIRMWARE_DIR/$fw" 2>/dev/null || true
    elif cmd curl; then
        curl -sL "$FIRMWARE_BASE/$fw" -o "$FIRMWARE_DIR/$fw" 2>/dev/null || true
    fi
done

# Also add common Intel/Broadcom firmware
INTEL_DIR="$ISO_ROOT/lib/firmware/iwlwifi"
mkdir -p "$INTEL_DIR"
...
start() {
    ...
    # Auto-connect to available WiFi if configured
    if [ -f /etc/wpa_supplicant/wpa_supplicant.conf ]; then
        wpa_supplicant -B -i wlan0 \
            -c /etc/wpa_supplicant/wpa_supplicant.conf 2>/dev/null || true
        udhcpc -i wlan0 -t 10 -T 3 2>/dev/null || true
    fi

    # Run scanner
    sh /platine/platine-scan.sh
    eend $?
}
SVCEOF
